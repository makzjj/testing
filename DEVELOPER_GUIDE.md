...
- Historical Firmware Integration Binary/Text parity is preserved by committed JSON fixtures under `tests/fixtures/firmware_catalogs/`. They are test-only, non-executable, and must not be imported by runtime code. Runtime/UI work should continue to use the canonical current controller/model metadata and BioBot orange styling.
