...
class ManualTextCommandDialog(QDialog):
    ...
    def __init__(self, controller: FirmwareIntegrationController, parent: QWidget | None = None) -> None:
        ...
        helper = QLabel(
            "Send one supported text command at a time through the Firmware Integration controller. "
            "TX/RX history is local to this dialog."
        )
        helper.setWordWrap(True)
        helper.setObjectName("ManualTextHelperText")
        ...
        self.command_combo.currentIndexChanged.connect(self._refresh_form_state)
        ...
        controls.addWidget(QLabel("Command"), 0, 0)
        controls.addWidget(self.command_combo, 0, 1, 1, 3)
        controls.addWidget(self.send_button, 0, 4)
        controls.addWidget(self.close_button, 0, 5)
        controls.addWidget(QLabel("Value"), 1, 0)
        controls.addWidget(self.value_input, 1, 1, 1, 5)
