...
from myconfig.constants import NODE_ID_MAPPING, BCMD_GET_NODE_ID
            ...
            with open(file_path, mode='w', newline='', encoding='utf-8') as csv_file:
                ...
                for line in raw_text.splitlines():
                    ...
                    if line.startswith("[") and "]" in line:
                        ...
                        elif "Node" in message and any(x in message for x in ["timed out", "detected", "responded"]):
                ...
                if data:
                    ...
                    for pkt in packets:
                        if pkt.get("status") == "ok":
                            ...
                            # Handle CAN-over-UART packets
                            if pkt.get("type") == "can_over_uart" and 'sender' in pkt:
                                node_id = pkt['sender']
                                cmd = pkt.get('cmd', 0)
                                params = pkt.get('params', [])
                                
                                # Forward to Communication Monitor if it's a test frame
                                if cmd == 0xBF: # BCMD_COMM_TEST_FRAME
                                    seq = (params[0] << 8) | params[1] if len(params) >= 2 else None
                                    if seq is not None and hasattr(self, 'comm_monitor'):
                                        self.comm_monitor.process_test_packet(node_id, seq)
                                elif cmd == 0xBD: # BCMD_COMM_TEST_FINISHED
                                    if hasattr(self, 'comm_monitor'):
                                        self.comm_monitor.handle_test_finished(node_id)
                                elif cmd == 0xC8: # BCMD_GET_VERSION
                                    _, version = decode_command(cmd, params)
                                    if hasattr(self, 'comm_monitor'):
                                        self.comm_monitor.handle_node_version(node_id, version)
                                param_str = " ".join(f"{b:02X}" for b in params)

                                msg = f"RX[CAN] From:{node_id:02X} Cmd:{cmd:02X} Params:[{param_str}]"
                                
                                # Add decoded TPOS info to log if available
                                if cmd == 0x81:
                                    _, decoded_value = decode_command(cmd, params)
                                    if decoded_value:
                                        msg += f" -> {decoded_value}"
                                ...
                                if 2 <= node_id <= 17:
                                    ...
                                    # Handle interrupt command specifically
                                    if cmd == 0xD8:  # GET_INTERRUPT command
                                        interrupt_data = parse_get_interrupt(params)

                                        # Store both the text and the full data
                                        if isinstance(interrupt_data, dict) and 'text' in interrupt_data:
                                            self.node_status[node_id]["interrupt"] = interrupt_data['text']
                                            self.node_status[node_id]["interrupt_data"] = interrupt_data
                                            self.log(f"✅ Interrupt status for Node {node_id}: {interrupt_data['text']}")
                                        else:
                                            # Fallback if something goes wrong
                                            self.log(f"❌ ERROR: Invalid interrupt data format for Node {node_id}")
                                            self.node_status[node_id]["interrupt"] = "Error"
                                            self.node_status[node_id]["interrupt_data"] = {}

                                    # ✅ Option 1: Use pre-decoded data from parser
                                    decoded_key = pkt.get("decoded_key")
                                    decoded_value = pkt.get("decoded_value")
                                    if decoded_key and decoded_value:
                                        self.node_status[node_id][decoded_key] = decoded_value
                                        if decoded_key != "tpos":
                                            self.log(f"Decoded [{decoded_key}] = {decoded_value}")

                                    # ✅ Option 2: Use local decode_command for redundancy
                                    # Import at top of file: from data.binary_cmd_parser import decode_command
                                    key, value = decode_command(cmd, params)
                                    
                                    if key == "move_yayb":
                                        pass # Handled by ACK
                                        
                                    elif cmd == 0xC9: # Initialization Acknowledge (Step 2)
                                        if params == [0x3A, 0x0B]:
                                            self.log(f"✅ Step 2: Received C9 Ack from Node {node_id:02d}")
                                            if self.init_state.get(node_id) == 1:
                                                self.init_state[node_id] = 2 # Step 3: Sending RUN
                                                try:
                                                    vel = int(self.velocity_input.text())
                                                    vel_bytes = list(vel.to_bytes(2, byteorder='big', signed=True))
                                                    self.log(f"⚙️ Step 3: Automatically sending RUN to Node {node_id:02d} (Velocity: {vel})")
                                                    self._send_single_node_command(node_id, "Init RUN", [0x88] + vel_bytes)
                                                except ValueError:
                                                    self.log("⚠️ Invalid velocity for Init Step 3")
                                        ...
                                        if value is not None:
                                            ...
                                            # Fallback for Auto-move: if Ready but stuck in moving state
                                            if value.get("text") == "Ready" and self.auto_move_btn.isChecked():
                                                target_node_id = self.motion_node_combo.currentData()
                                                if node_id == target_node_id:
                                                    # Safety: Ignore Ready signals that arrive too soon after sending (stale packets)
                                                    time_since_send = time.time() - self.move_sent_time.get(node_id, 0)
                                                    if time_since_send < 0.5:
                                                        return # Stale packet from previous move
                                                        
                                                    if node_id in self.last_positions and self.auto_move_state != 0:
                                                        self.log(f"⚠️ Node {node_id:02d} is Ready but no TPOS 'E/N' received. Recovery triggered.")
                                                        
                                                        # Track as missed TPOS E in visualization
                                                        if node_id in self.motor_panels:
                                                            target_pos = int(self.start_pos_input.text()) if self.auto_move_state == 1 else int(self.end_pos_input.text())
                                                            offset = self.last_positions[node_id] - target_pos
                                                            self.motor_panels[node_id].increment_miss_e(offset)
                                                        
                                                        # Manually trigger tpos check with last known position as 'N'
                                                        self.on_tpos_received(node_id, ('N', self.last_positions[node_id]))
                                                    else:
                                                        self.log(f"🔍 DEBUG: Node {node_id:02d} is Ready, state={self.auto_move_state}")

                                            # Filter periodic system mode logs if requested
                                            if not getattr(self, 'hide_sys_mode_chk', None) or not self.hide_sys_mode_chk.isChecked():
                                                self.log(f"System Mode Response (CAN): {value['text']}")
                                    
                                    elif key == "tpos":
                                        self.on_tpos_received(node_id, value)
                                        
                                    elif cmd == 0xB9: # BCMD_SW_SCAN_CONTINUE
                                        if len(params) >= 5:
                                            type_char = chr(params[0])
                                            pos = int.from_bytes(params[1:5], byteorder='big', signed=True)
                                            if type_char == 'E' and self.is_scanning_continue and node_id == self.scan_target_node:
                                                self.log(f"📥 Received Scan Final ACK (0xB9 'E') from Node {node_id:02d}")
                                                self.scan_last_elapsed = time.time() - self.scan_start_time
                                                self.finalize_scan()
                                            elif type_char == 'S':
                                                self.log(f"📥 Received Scan Start ACK (0xB9 'S') from Node {node_id:02d} at {pos}")
                                        
                                    elif key == "getpos":
                                        if value and isinstance(value, tuple):
                                            self.log(f"📥 Received GETPOS from Node {node_id:02d}: {value[1]}")
                                            self.on_tpos_received(node_id, value)
                                        
                                    elif key == "tpos_done":
                                        # Reset the sent flag when we get the actual 'E' response
                                        self.m_tpos_done_sent[node_id] = True

                                    if key and key != decoded_key:  # Only log if different from parser
                                        self.node_status[node_id][key] = value
                                        if key != "tpos" and key != "sys_mode":
                                            self.log(f"Decoded [{key}] = {value}")

                                    # Forward ZPOSS samples to plot window when present
                                    if 'adc_raw' in pkt and 'physical_value' in pkt:
                                        try:
                                            if hasattr(self, 'zposs_plot') and self.zposs_plot:
                                                self.zposs_plot.update_plot_data(pkt['adc_raw'], pkt['physical_value'])
                                        except Exception as e:
                                            self.log(f"❌ Failed to update ZPOSS plot: {e}")

                                    # Forward ToF samples to ToF plot window
                                    if key == "tof_distance" or decoded_key == "tof_distance":
                                        try:
                                            tof_data = value if key == "tof_distance" else decoded_value
                                            if hasattr(self, 'tof_plot') and self.tof_plot:
                                                self.tof_plot.update_plot_data(tof_data)
                                        except Exception as e:
                                    ...
                                    if pkt.get("uuid_response"):
                                        ...
                                        is_valid = pkt.get("uuid_valid", False)
                                        if is_valid:
                                            self.log(f"✅ Node {node_id:02X} UUID: {uuid}")
                                            self.node_status[node_id]["uuid"] = uuid
                                            self.node_status[node_id]["uuid_valid"] = True
                                        else:
                                            self.log(f"❌ Node {node_id:02X} UUID: Invalid")
                                            self.node_status[node_id]["uuid_valid"] = False


                                    # Process Node ID response
                                    if cmd == BCMD_GET_NODE_ID:
                                        self.process_node_id_response(node_id, params)

                                self.log(msg)

                            # Handle direct UART packets (MCU version, stats)
                            elif pkt.get("type") == "direct_uart":
                                ...
                                # Use decode_command for generic parsing
                                key, value = decode_command(cmd, params)
                                if key == "sys_mode":
                                    self.sys_mode = value
                                    self.update_status_ui()
                                    if not getattr(self, 'hide_sys_mode_chk', None) or not self.hide_sys_mode_chk.isChecked():
                                        self.log(f"System Mode Response (Direct): {value['text']}")
                                
                                if cmd == 0xBC: # COMM_STATS
                                    from data.binary_cmd_parser import parse_comm_stats
                                    # Skip the 0xBC command byte
                                    stats = parse_comm_stats(payload[1:])
                                    if stats and hasattr(self, 'comm_monitor'):
                                        self.comm_monitor.handle_comm_stats(stats)
                                
                                if pkt.get("mcu_version_response"):
                                    version = pkt.get("mcu_version", "Unknown")
                                    # Only update if it's a new version or we don't have one yet
                                    if version != self.mcu_version:
                                        ...
                                        self.log(f"ℹ️ MCU version already set to: {version}")
